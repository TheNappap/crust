...
impl BlockDefinition for Call {
    ...
    fn parse(&self, header: Vec<Token>, _body: Vec<Token>, parser: &Parser) -> Result<Expression> {
        ...
        use Token::*;
        use Delimeter::*;
        let (name, tokens) = match header.as_slice() {
            [Ident(name), Group(Parens, tokens)] => (name.to_owned(), tokens),
            [Ident(type_name), Operator(ColonColon), Ident(name), Group(Parens, tokens)] => 
                (type_name.to_owned() + "__" + name, tokens),
        ...
        let exprs = parser.parse_list(tokens.clone())
            ...
            .try_collect()?;
        Ok(Expression::Call(Signature::new(&name, vec![], Type::Inferred), exprs))
    }
